...
def resolve_capsule_world(cap: Capsule, world) -> tuple[np.ndarray, bool]:
...
def test_capsule_ground_collision() -> None:
    ...
    cap_py = Capsule(np.array([0.0, 0.2, 0.0], dtype=np.float32), 0.9, 0.3)
    ...
    assert cap.center.y == pytest.approx(1.2, abs=1e-4)
