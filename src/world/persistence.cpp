...
ChunkStore::ChunkStore(const std::string &root, const std::string &codec,
                       bool use_rle, int threads)
...
std::filesystem::path ChunkStore::_region_dir(int cx, int cz) const {
  std::filesystem::path d =
      root_ / ("r." + std::to_string(cx / 32) + "." + std::to_string(cz / 32));
  std::filesystem::create_directories(d);
  return d;
}

std::filesystem::path ChunkStore::chunk_path(int cx, int cz) const {
  return _region_dir(cx, cz) /
         ("c." + std::to_string(cx) + "." + std::to_string(cz) + ".bin");
}
...
ChunkStore::rle_decode(const std::vector<std::uint8_t> &vals,
...
std::vector<std::uint8_t>
ChunkStore::_compress(const std::vector<std::uint8_t> &data) const {
  if (codec_ == "zstd" && VOXELVK_HAS_ZSTD) {
    size_t max_size = ZSTD_compressBound(data.size());
    std::vector<std::uint8_t> out(max_size);
    size_t csize =
        ZSTD_compress(out.data(), max_size, data.data(), data.size(), 10);
  ...
  if (codec_ == "lz4") {
    ...
    std::vector<std::uint8_t> out(max_size);
    size_t csize = LZ4F_compressFrame(out.data(), max_size, data.data(),
                                      data.size(), nullptr);
...
std::vector<std::uint8_t>
ChunkStore::_decompress(const std::vector<std::uint8_t> &data) const {
  if (codec_ == "zstd" && VOXELVK_HAS_ZSTD) {
  ...
  if (codec_ == "lz4") {
    ...
    size_t result =
        LZ4F_decompress(dctx, out.data(), &dst_size, src, &src_size, nullptr);
    LZ4F_freeDecompressionContext(dctx);
...
void ChunkStore::save_chunk_sync(const Chunk &chunk) {
  ...
  if (static_cast<std::size_t>(chunk.size * chunk.height * chunk.size) !=
      vox.size()) {
  ...
  if (use_rle_) {
    ...
    if (vals.size() > static_cast<std::size_t>(
                          std::numeric_limits<std::int32_t>::max()) ||
        counts.size() > static_cast<std::size_t>(
                            std::numeric_limits<std::int32_t>::max())) {
      throw std::runtime_error("RLE output too large");
    }
    ...
    data.insert(data.end(),
                reinterpret_cast<const std::uint8_t *>(counts.data()),
                reinterpret_cast<const std::uint8_t *>(counts.data()) +
...
void ChunkStore::save_async(const Chunk &chunk) {
  futures_.emplace_back(std::async(std::launch::async,
                                   [this, chunk] { save_chunk_sync(chunk); }));
}
...
std::optional<ChunkData> ChunkStore::load_chunk(int cx, int cz) {
  ...
  std::vector<std::uint8_t> blob((std::istreambuf_iterator<char>(ifs)),
  ...
  if (nvals > 0) {
    ...
    const std::int32_t *cnt_ptr =
        reinterpret_cast<const std::int32_t *>(ptr + nvals);
...
void ChunkStore::wait_all() {
  for (auto &f : futures_) {
    try {
      ...
      std::cerr << "[ChunkStore::wait_all] Exception in async save: "
                << e.what() << std::endl;
    } catch (...) {
      std::cerr << "[ChunkStore::wait_all] Unknown exception in async save."
                << std::endl;
    }
  }
  futures_.clear();
