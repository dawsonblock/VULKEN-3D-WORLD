...
def ortho_from_points(points, light_dir, margin=10.0):
    ...
    z = -light_dir / np.linalg.norm(light_dir)
    x = np.cross(up, z)
    x /= np.linalg.norm(x)
    y = np.cross(z, x)
    ...
    V[:3, 3] = 0
    pts_lv = (
        V @ np.hstack([points, np.ones((points.shape[0], 1), np.float32)]).T
    ).T[:, :3]
    mn = pts_lv.min(0) - margin
    mx = pts_lv.max(0) + margin
    proj = np.eye(4, dtype=np.float32)
    proj[0, 0] = 2.0 / (mx[0] - mn[0])
    proj[1, 1] = 2.0 / (mx[1] - mn[1])
    proj[2, 2] = -2.0 / (mx[2] - mn[2])
    proj[0, 3] = -(mx[0] + mn[0]) / (mx[0] - mn[0])
    proj[1, 3] = -(mx[1] + mn[1]) / (mx[1] - mn[1])
    proj[2, 3] = -(mx[2] + mn[2]) / (mx[2] - mn[2])
    return proj @ V
