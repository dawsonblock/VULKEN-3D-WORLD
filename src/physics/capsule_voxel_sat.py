"""Collision detection helpers for resolving capsules against voxel worlds."""
...
class WorldProtocol(Protocol):
    """Minimal interface for the voxel world used in tests."""

    def get_block_at_world_position(self, x: float, y: float, z: float) -> int:
        """Return a non-zero value when the block at ``(x, y, z)`` is solid."""
        ...  # pragma: no cover - simple protocol
...
def resolve_capsule_world(
    ...
    Returns the total offset applied to the capsule and whether it touched the
    ground during the resolution process.
    """
    ...
    for _ in range(max_iters):
        # Compute voxel bounds surrounding the capsule's current position.
