"""Minimal capsule vs. voxel collision helpers used in unit tests."""
...
class WorldProtocol(Protocol):
    """Protocol for the tiny voxel world used in the tests."""
...
def capsule_box_penetration(
    ...
    """Check whether ``cap`` penetrates the AABB ``mn``/``mx``.

    Returns a tuple ``(hit, normal, penetration)``.  ``normal`` is ``None`` when
    ``hit`` is ``False``.
    """
    ...
    if pen > 0.0:
        ...
        return True, cast(np.ndarray, n), float(pen)
...
def resolve_capsule_world(
    ...
    """Resolve ``cap`` against ``world`` and return the applied offset.

    The world is expected to provide ``get_block_at_world_position`` returning a
    truthy value for solid voxels.  The algorithm is a very small SAT-based
    solver suitable for the unit tests only.
    """
