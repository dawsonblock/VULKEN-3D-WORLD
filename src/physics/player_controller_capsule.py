...
class PlayerControllerCapsule:
    """Simple capsule-based player controller."""
    ...
    def update(self, dt: float, forward: np.ndarray, right: np.ndarray) -> None:
        ...
        hv = self.vel.copy()
        hv[1] = 0.0
        self.vel += (wish*target - hv) * min(1.0, (GROUND_ACCELERATION if self.on_ground else AIR_ACCELERATION)*dt)
        ...
        moving = self.input["f"] or self.input["b"] or self.input["l"] or self.input["r"]
        if np.allclose(off, 0.0, atol=1e-6) and moving:
