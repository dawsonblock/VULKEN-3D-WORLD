...
bool PBRPass::init(VkPhysicalDevice /*phys*/, VkDevice dev, VkFormat colorFormat, VkFormat depthFormat){
    device = dev;
    ...
    lci.bindingCount = 6; lci.pBindings = binds;
    if(vkCreateDescriptorSetLayout(device,&lci,nullptr,&materialSetLayout)!=VK_SUCCESS) return false;

    VkPushConstantRange pcr{};
    pcr.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
    pcr.offset = 0;
    pcr.size = sizeof(PBRPushConstants);
    VkPipelineLayoutCreateInfo plci{VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO};
    plci.setLayoutCount = 1; plci.pSetLayouts = &materialSetLayout;
    plci.pushConstantRangeCount = 1; plci.pPushConstantRanges = &pcr;
    if(vkCreatePipelineLayout(device,&plci,nullptr,&pipelineLayout)!=VK_SUCCESS) return false;

    VkShaderModule vs = loadShader(device, "spv/lighting/pbr_lit.vert.spv");
    VkShaderModule fs = loadShader(device, "spv/lighting/pbr_lit.frag.spv");
...
void PBRPass::record(VkCommandBuffer cmd, VkDescriptorSet materialSet, const PBRPushConstants& pc, uint32_t vtxCount){
    vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &materialSet, 0, nullptr);
    vkCmdPushConstants(cmd, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(PBRPushConstants), &pc);
